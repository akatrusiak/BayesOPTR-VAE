...
if __name__ == '__main__':
    ...
    name = args.name
    # assume the script is launched from the same directory as init.sh
    cwd = os.getcwd()
    # always write output files into ./simulation/transoptr/<name> relative to cwd
    output_dir = os.path.join(cwd, 'simulation', 'transoptr', name)
    
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    tune_config = os.path.join(output_dir, 'tune_config.xml')
