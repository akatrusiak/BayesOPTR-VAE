...
def parse_elements(trimmed_tree, datadat_path, apertures=None,
    ...
    for seq in sequences:
        for elem in seq.findall('element'):
            ...
            if optr_tag is not None and 'aperR' in optr_tag.attrib:
            ...
            for epics in elem.findall('epics'):
                for setpoint in epics.findall('setpoint'):
                    if 'min' not in setpoint.attrib:
                        # No tuning range — record location only
                        data[elem_id] = {
                            'type': ele_type,
                            'loc': loc,
                        }
                    ...
                    # Use PV name as key (beam-config style)
                    # DO NOT DO THIS, steerers do not get the :CUR suffix in the sim
                    ...
                    # Quad key gets :CUR suffix for data.dat lookup
                    ...
                    try:
                        start_val = datadat.get_element(name=key)['value']
                    except Exception:
